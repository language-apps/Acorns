...
function CategoryRelatedPhrases(acornsObject, data, opt)
{
	...
	this.checkAnswer = function(text, indigenous, point, silent)
	{
		...
		var sentence = this.formatTextField(indigenous,  point);
		var distance = acorns.system.stringDistance(text, sentence);
		var sLower = text.toLowerCase();
		var cLower = sentence.toLowerCase();
...
function MissingWord(acornsObject, data)
{
	...
	this.makeButtonPanel = function()
	{
    	...
    	var rowPanel, button, i, j, src, result, printable, buttons=false;
	    var specials = this.getSpecials();
		specialsArray = specials.trim().split(/\s+/);
		var len = specialsArray.length;
    	for (i=0; i< len; i++)  
    	{
			src = specialsArray[i];
			...
			{
				printable = String.fromCharCode(src.charAt(j));
				if (printable == NaN) 
				{
					result = false;
					break;
    ...
    this.isPlayable = function(layer) 
	{
		...
		specials = layerData.getSpecials();
	...
	this.configurePlayPanel = function(panel) 
	{
		...
		var buttonPanel = this.makeButtonPanel(panel.clientHeight);
   ...
   var searchJSON = function(tags, key)
   {
	...
	var findModifierIndex = function(modifier)
	{
	  ...
	  for (index=0; index < modifiers.length; index++)
	  {
		  ...
		  return modifiers[index].modIndex;
	  }
	  return -1;
	}
...
function Layer(nameL, valueL, languageL, alignL, excludedL, specialsL)
{
   ...
   var specials = (specialsL==null) ? "" : specialsL
...
{ 
	...
	this.parseRelatedCategory = function(acorns, lesson, lessonObject)
	{
		...
		{
		    layer  = new Layer(layers[layerNo].getAttribute("name"), layers[layerNo].getAttribute("value"),
			   layers[layerNo].getAttribute("language"), null, null, layers[layerNo].getAttribute("specials"));
			getParams(layers[layerNo], layer);
